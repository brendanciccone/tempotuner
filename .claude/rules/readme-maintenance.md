...
| **Test counts** (### Test Types) | Tests are added or removed — update the counts and description lists |
| **Recent Additions** | Any shipped change worth calling out — add a bullet under the current month's heading (create the heading if it doesn't exist) |
...
2. **Keep counts accurate** — when adding tests, grep for the current count strings (e.g., "165 tests") and update them to the new totals.
3. **Match existing style** — use the same emoji-prefixed bullet format, heading hierarchy, and tense already in the README.
4. **Don't bloat** — only add entries that would help a new contributor or reviewer understand what the project does and what changed. Skip purely internal refactors unless they alter observable behavior.
