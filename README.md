...
- **Unit** (`tests/unit/`) — 111 tests covering audio processing, note detection, tuner initialization, metronome accents and beat scheduling (the downbeat differs in timbre and not only in gain, a compound meter groups in threes, a bad beat index or a non-finite AudioContext time throws rather than propagating a NaN into a timer, and a beat scheduled 100ms ahead of the clock paints 100ms later), the beat indicator (one cell per beat, the sounding beat lit zero-based, nothing lit while stopped, before the first click of a run sounds, or when the reported beat falls outside the measure, accents read from the same map the clicks use, and the row hidden from assistive tech behind a single static statement of the meter), tuning-meter geometry (the needle lands exactly on the zone edge at the in-tune boundary, clamps at full scale, and agrees with the classifier across a swept ±20 cent range), tap-tempo input (one tap per keyboard activation and per pointer press, no tap when another control is activated, and the two-second reset), the `cn()` class merger (the custom `text-micro` and `tracking-*` tokens resolve as the scales they belong to, and ordinary conflicts still resolve), the theme regression guards (single theme, no `data-style` variants, no light/dark pair, no second hue in the UI, the 18px bitmap floor, the contrast gates: each stop against `--screen`, AA on all three surfaces, inverse-video legibility, ramp monotonicity, and the persistence model: both decay durations inside the UI caps, only the OFF edge decaying, and nothing geometric in the transition list), and the `useTuner` hook orchestration (including secure-context detection, mediaDevices feature detection, sample-rate fallback, DOMException-name error mapping, the cleanup→re-initialize retry path, a regression guard that `cleanup()` awaits `AudioContext.close()`, and the iOS Safari `needs-gesture` → `startWithGesture` flow)

- **Security** (`tests/security/`) — 13 tests guarding the dependency floors that close Dependabot alerts, since those fixes live in `pnpm.overrides` or a pinned version rather than in any code this repo owns and would otherwise regress invisibly. Five assert the resolved `browserslist` behaves as the patched release does — a `__proto__` key in custom stats neither crashes `getStat` nor writes the result's prototype, ordinary browsers still normalise, and the version sits at or above the 4.28.7 floor. The rest read the resolved versions out of `pnpm-lock.yaml` and hold `next` at 16.3.3+, `sharp` at 0.35.4+ and `js-yaml` at 4.3.2+, across every entry rather than the first one found, with the floor comparison itself under test (16.2.12 reads as below 16.3.3, not above it) and a guard that each package is still present, so a rename cannot pass the check by leaving it nothing to compare

## Recent Additions

### September 2026

- Upgraded Next.js to 16.3.4 to close two critical unauthenticated remote-code-execution advisories, and raised two transitive security floors alongside it. Both Next.js advisories are vulnerable across `>=16.0.0 <16.3.3` — GHSA-p293-qw3h-jr36 on Windows-hosted servers and GHSA-2xp9-vwfh-vxw4 in the Image Optimization API when AVIF files are used — so 16.3.3 is the first patched release on the 16.x line and 16.2.x closes neither. The `sharp` override moves to `^0.35.4`, the release that picks up the patched libheif (GHSA-rgj7-g3m4-5g8c, carrying libheif's GHSA-g89c-p67h-r497 and GHSA-2jg2-4ch7-h545), and the `js-yaml@^4.0.0` override to `^4.3.2`, where `maxTotalMergeKeys` finally bounds CPU use across empty merge sources (GHSA-2883-xcg3-v3hh). The matching `eslint-config-next` is kept on the same release as `next`. Together these close Dependabot alerts #83–#88.

- Neither RCE is reachable from the deployed site, but that is a property of the current host rather than of the code: the Cloudflare deployment is a static export (`output: 'export'`), which emits no server and no `/_next/image` route, so there is nothing running to reach the Windows path handler or the AVIF optimizer. The documented Railway fallback is the reason to patch anyway — `RAILWAY_ENVIRONMENT` switches the build back to `standalone`, which does run the Next.js server and does serve Image Optimization, so on that path both advisories are live. Upgrading, rather than leaning on the export, is what keeps the fallback safe to take.

- Regenerated `pnpm-lock.yaml` with `pnpm install --lockfile-only` rather than editing the package keys by hand. Hand-editing a version in place leaves the old `resolution` integrity hash attached to the new tarball and drops the resolution records for renamed platform packages entirely, which fails a clean `pnpm install --frozen-lockfile` twice over: `ERR_PNPM_TARBALL_INTEGRITY` on every rewritten artifact, then `Cannot use 'in' operator to search for 'directory' in undefined` once the headless installer tries to build a dependency graph from the incomplete entries. That is a CI- and deploy-only failure — it never shows up in a local `pnpm test` against an already-populated `node_modules` — so the check that matters after any dependency change is `pnpm install --frozen-lockfile` on a clean store.

- Added a `browserslist@^4.0.0` override flooring at `^4.28.7`, closing both open high-severity Dependabot alerts. The lockfile had been sitting on 4.24.4 since January 2025, and 4.28.7 is the release that fixes both. `normalizeStats` built its result as a `{}` literal and tested known browsers with a bare `data[i]` lookup; because `browserslist-stats.json` is read with `JSON.parse`, a literal `"__proto__"` key arrives as a real own, enumerable property that `for..in` walks, at which point `data["__proto__"]` resolves up the chain to `Object.prototype` — truthy, so the branch is taken — and `data["__proto__"].versions.length` throws an uncaught `TypeError` mid-build; with two versions in the payload the other branch is taken instead and `normalized["__proto__"] = stats["__proto__"]` invokes the `__proto__` setter, swapping the returned object's prototype for attacker-supplied data rather than storing a key. 4.28.7 builds with `Object.create(null)` and gates on `hasOwnProperty.call(data, i)`, so the crash is gone and `__proto__` lands as an ordinary own property. Separately, both query caches were plain objects that only ever grew — 20,000 distinct queries retained ~8.6MB that nothing could release — and are now `Map`s capped at 500 entries with FIFO eviction, measuring flat across the same run. Both dependents (`autoprefixer`, which declares `^4.24.4`, and `@babel/helper-compilation-targets` at `^4.24.0`) already accepted the patched line, so the override pins a floor rather than lifting a pin, and the version-scoped selector leaves a future 5.x unaffected. Neither surface is reachable here. browserslist runs only at build time, and only behind Babel: `autoprefixer` is declared in `package.json` but never wired into `postcss.config.mjs`, which loads `@tailwindcss/postcss` alone, so Tailwind v4 prefixes through Lightning CSS against its own targets and never asks browserslist anything. The repo also ships no `browserslist-stats.json` and no `browserslist` config, and a build issues a handful of queries rather than an unbounded stream. Building `out/` before and after the bump confirms it: the emitted CSS is byte-identical (same content hash, same chunk filename) and the two trees differ only by Next.js's randomised per-build id. Because the fix lives in `pnpm.overrides` rather than in code this repo owns, a new `tests/security/` suite guards it: a regenerated lockfile or a dropped override would otherwise reintroduce both bugs with nothing failing. The five tests assert the resolved dependency's behaviour rather than its version string alone — three of them fail against 4.28.6 and pass against 4.28.7

### August 2026

- Added a visual beat indicator to the metronome and fixed the three things that made it worth adding. The measure is now drawn as one cell per beat — a sounding beat is a solid discharge block, the downbeat the brightest thing in the row, and at rest the accented cells stay legible so a 6/8 reads as 3+3 before the metronome is even running. It is useful with the volume down and it is the only way to see where the accents fall in an odd meter. Two defects sat underneath it. The beat was painted at the moment its click was *scheduled*, and the scheduler runs 100ms ahead of the AudioContext clock, so the display led the sound by up to 40% of a beat at 240 BPM; the paint is now queued for the click's own start time, and the pending paints are cancelled on stop, on a time-signature change and on unmount. Starting the metronome had the same fault in miniature: it reported beat 0 immediately, so the downbeat cell lit about 60ms before the click it belonged to, and the scheduled paint that should have lit it was a no-op against a value already set. The gap before the first click now reports no beat at all. The metronome also reported the *next* beat rather than the sounding one, which the tap pad compensated for by flashing on index 1 — both now agree that the downbeat is 0. Separately, compound accents were read from state captured by the `setTimeout` chain at the moment it started, so switching 4/4 to 6/8 mid-run kept clicking flat until you stopped and restarted; that value moves to a ref like every other value the scheduler reads. The accent map and the click voices are now one pure module shared by the audio and the display, so the panel cannot show a downbeat where the click does not put one, and the per-beat `console.log` block (five lines per click, four times a second at 240 BPM) is gone

- Gave the panel phosphor persistence. Every lit control snapped both on and off, which no emitter does: drive arriving is instant, drive stopping is a relaxation. `.ac-lamp` now carries a 90ms decay on colour, border and glow, and every selector meaning "currently driven" zeroes the duration, so the direction of the edge chooses between an instant frame and the tail. It is applied to the soft keys, the tabs, the tap pad, the tuning status strip and the new beat cells. `--decay` is 90ms and `--decay-fast`, used by disabled controls, is 60ms. Both are chosen against the framework's UI ceilings (250ms for anything a user waits on, 60ms for a control de-energizing) rather than against a decay figure for this tube, which is a look rather than a part number; inventing a spectral tail for it would be inventing a measurement. Nothing in the transition list moves or resizes, so it stays on under `prefers-reduced-motion`
