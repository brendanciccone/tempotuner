...
- **Security** (`tests/security/`) — 5 tests guarding the dependency floors that close Dependabot alerts, since those fixes live in `pnpm.overrides` rather than in any code this repo owns and would otherwise regress invisibly. They assert the resolved `browserslist` behaves as the patched release does — a `__proto__` key in custom stats neither crashes `getStat` nor writes the result's prototype, ordinary browsers still normalise, and the version sits at or above the 4.28.7 floor

## Recent Additions

### September 2026

- Upgraded Next.js to 16.2.12 to patch the unauthenticated Windows-server and AVIF Image Optimization remote-code-execution vulnerabilities. The matching ESLint preset is kept on the same release. The transitive security floors now require sharp 0.35.4, which carries the patched libheif build, and js-yaml 4.3.2, which bounds work across empty merge sources. These changes close Dependabot alerts #83–#88 in both the manifest and lockfile.
